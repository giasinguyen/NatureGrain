...
@Repository
public interface OrderDetailRepository extends JpaRepository<OrderDetail, Long> { // Find order details by product id -
                                                                                  // using explicit JPQL to avoid
       ...
       // Find average order processing time - using a default processing time since
       // update_at isn't available
       @Query(value = "SELECT 4.5", nativeQuery = true)
       ...
       List<Object[]> findOrderStatusDistributionByDate(@Param("startDate") Date startDate,
